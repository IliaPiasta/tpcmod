...
export default function Commands() {
  ...
  // Хранение выбранных команд
  const [selectedCommands, setSelectedCommands] = useState([]);

  // Функция для обработки клика по команде
  const toggleCommand = (command) => {
    setSelectedCommands((prev) =>
      prev.includes(command)
        ? prev.filter((c) => c !== command) // Убираем, если уже есть
        : [...prev, command] // Добавляем, если нет
    );
  };
    ...
    <>
      <div>
        ...
        <p className="text-white text-sm text-opacity-50 mb-5">
          You can get information about the commands of the TpC Bot.
        </p>
                  ...
                  {commands.map((command) => (
                    <Menu.Item key={command}>
                      <button
                        ...
                        className={`w-full rounded-lg p-2 cursor-pointer flex items-center space-x-3 transition-all duration-200 ${
                          selectedCommands.includes(command)
                            ? "bg-amber-500 text-white font-bold" // Подсветка выбранных команд
                            : "text-white"
                        }`}
                      >
                        <>
